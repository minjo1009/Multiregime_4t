# ci/wfo_entry.py  (v6: add __init__.py + sys.path injection)
...
def unzip_codepack_if_any(workspace="."):
    ...
    for pkg in ["strategy", os.path.join("strategy","v2"), "backtest"]:
        ...
        if os.path.isdir(pkg_path):
            for piece in [pkg_path]:
                initp = os.path.join(piece, "__init__.py")
                if not os.path.exists(initp):
                    try:
                        open(initp, "w", encoding="utf-8").write("# auto-generated by wfo_entry\n")
                    except Exception:
                        pass
...
def run_script_with_argv(py_path, argv):
    ...
    try:
        ...
        for p in [ws, sd, os.path.join(ws,"src")]:
            if p and p not in sys.path:
                sys.path.insert(0, p)
        print("[wfo_entry] sys.path[0:3] =", sys.path[:3])
        sys.argv = argv
